...
class UserService extends ServiceMap.Service<UserService, UserServiceShape>()("app/UserService") {
...
export class UserService extends ServiceMap.Service<UserService, UserServiceShape>()(
  ...
  // ── 3. Static layer ─────────────────────────────────────
  static readonly layer: Layer.Layer<UserService, never, UserRepo | CacheService> = Layer.effect(
    UserService,
    Effect.gen(function* () {
      const repo = yield* UserRepo;
...
interface OrderServiceShape {
  ...
  ) => Effect.Effect<Order, OrderCreateError | InsufficientInventoryError>;
...
).pipe(Layer.provide(DatabaseLive), Layer.provide(RedisLive));
```
...
const OrderServiceLive = OrderService.layer.pipe(Layer.provide(UserService.layer));
...
const handler = {
  fetch(request: Request, env: Env) {
    return program.pipe(Effect.provide(KVNamespace.layerFrom(env.MY_KV)), Effect.runPromise);
  },
};
```
...
class UserRepoTest extends ServiceMap.Service<UserRepoTest, UserRepoTestShape>()("test/UserRepo") {
  static readonly layer = Layer.effect(
...
).pipe(Layer.provide(TestRepos), Layer.provide(CacheServiceTest.layer));
