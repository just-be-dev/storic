...
export const CreateUserInput = Schema.Struct({
  ...
  name: Schema.String.pipe(Schema.check(Schema.isMinLength(1), Schema.isMaxLength(100))),
  organizationId: OrganizationId,
...
export const PositiveFromNumber = Schema.Number.pipe(
  Schema.decodeTo(Schema.Number.pipe(Schema.brand("Positive")), {
    decode: SchemaGetter.transformOrFail((n) =>
      n > 0 ? Effect.succeed(n) : Effect.fail(SchemaIssue.make("Must be positive")),
    ),
...
```typescript
// String checks
const Name = Schema.String.pipe(Schema.check(Schema.isMinLength(1), Schema.isMaxLength(100)));

const Email = Schema.String.pipe(Schema.check(Schema.isPattern(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)));

const ZipCode = Schema.String.pipe(Schema.check(Schema.isPattern(/^\d{5}(-\d{4})?$/)));
...
const Quantity = Schema.Number.pipe(Schema.check(Schema.isInt(), Schema.isGreaterThan(0)));
...
```typescript
const isEven = Schema.makeFilter<number>((n) => (n % 2 === 0 ? undefined : "Must be even"));
...
export const CreateOrderInput = Schema.Struct({
  productId: ProductId.pipe(Schema.annotate({ description: "The product to order" })),
...
export const UserPreferences = Schema.Struct({
  ...
  language: Schema.String.pipe(Schema.withDecodingDefault(() => "en")),

  // Constructor default — only affects `new Class(...)` / `makeUnsafe`, not decoding
  notificationsEnabled: Schema.Boolean.pipe(Schema.withConstructorDefault(() => Option.some(true))),
