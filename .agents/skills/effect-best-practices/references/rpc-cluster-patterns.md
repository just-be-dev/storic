...
**Always use explicit error unions** for RPC error types. Define errors with `Schema.TaggedErrorClass` (v4):
...
export class InsufficientInventoryError extends Schema.TaggedErrorClass<InsufficientInventoryError>()(
...
export class PaymentFailedError extends Schema.TaggedErrorClass<PaymentFailedError>()(
    "PaymentFailedError",
    {
        message: Schema.String,
...
// Explicit union of possible errors in the RPC definition
create: Rpc.mutation({
...
Define activity errors with `Schema.TaggedErrorClass` (v4) and include a `retryable` flag:
...
Use `ServiceMap.Service` (v4) for the service definition. Yield dependencies inside the layer effect, not via `accessors` or `dependencies` options:
