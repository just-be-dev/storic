...
const AppLive = Layer.mergeAll(UserServiceLive, OrderServiceLive, NotificationServiceLive).pipe(
...
const MainLive = DatabaseLive.pipe(Layer.provideMerge(ConfigLive), Layer.provideMerge(LoggerLive));
```
