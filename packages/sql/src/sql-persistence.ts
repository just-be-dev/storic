...
import { compileFilters, validateFieldName } from "./filter-sql.ts";
 ...
 */
export const sqlPersistenceLayer: Layer.Layer<Persistence, PersistenceError, SqlClient> =
  Layer.effect(
    Persistence,
    Effect.gen(function* () {
      ...
      const put = (record: PersistenceRecord) =>
        ...
        `;
          ...
          if (rows.length === 0) {
            return yield* new PersistenceError({
              message: `Put failed: entity ${record.id} not found after insert`,
            });
          }
      ...
      const get = (id: string) =>
        Effect.gen(function* () {
          const rows = yield* sql<Record<string, unknown>>`SELECT * FROM entities WHERE id = ${id}`;
      ...
      const query = (params: QueryParams) =>
        Effect.gen(function* () {
          const compiled = yield* compileFilters(params.filters);
          ...
          const rows = yield* sql.unsafe<Record<string, unknown>>(stmt, bindValues);
      ...
      const update = (
        ...
        `;
          ...
          if (rows.length === 0) {
            return yield* new PersistenceError({
              message: `Update failed: entity ${id} not found after update`,
            });
          }
          ...
          .withTransaction(
            Effect.gen(function* () {
              ...
              for (const entry of params.patches) {
                const compiled = yield* compileFilters(entry.filters);
