 ...
 * ```
 */
export class AutomergeDocs extends ServiceMap.Service<
  AutomergeDocs,
  AutomergeDocsShape
 ...
 */
export const automergePersistenceLayer: Layer.Layer<
  Persistence,
  PersistenceError,
  AutomergeDocs
> = Layer.effect(
  Persistence,
  Effect.gen(function* () {
    const { catalog: catalogRef, entities: entitiesRef } =
      yield* AutomergeDocs;
    ...
    const query = (params: QueryParams) =>
      Effect.gen(function* () {
        ...
        for (const id of candidateIds) {
          ...
          if (!matchesFilters(entityDoc.data as Record<string, unknown>, params.filters)) continue;
    ...
    const update = (
      ...
      Effect.uninterruptible(
        Effect.gen(function* () {
          ...
          // Update catalog: metadata + indexes
          const oldIndexed = toPlain(oldEntry.indexedValues ?? {}) as Record<
            string,
            string
          >;
        ...
        Effect.gen(function* () {
          ...
          for (const entry of params.patches) {
            ...
            for (const id of Object.keys(candidateIds)) {
              ...
              if (!matchesFilters(entityDoc.data as Record<string, unknown>, entry.filters)) continue;
              ...
              patched.push({
                ...
                oldIndexed: toPlain(catEntry.indexedValues ?? {}) as Record<
                  string,
                  string
                >,
              });
    ...
    const remove = (id: string) =>
      Effect.uninterruptible(
        Effect.gen(function* () {
          ...
          // Remove from catalog
          if (entry) {
            const indexed = toPlain(entry.indexedValues ?? {}) as Record<
              string,
              string
            >;
            yield* Ref.update(catalogRef, (doc) =>
 ...
 * ```
 */
export const saveState: Effect.Effect<SavedState, never, AutomergeDocs> =
  Effect.gen(function* () {
    const { catalog, entities } = yield* AutomergeDocs;
    const catalogDoc = yield* Ref.get(catalog);
    const entityMap = yield* Ref.get(entities);
...
export class AutomergePersistence {
  ...
  static layer(): Layer.Layer<Persistence | AutomergeDocs, PersistenceError> {
    ...
    return Layer.mergeAll(
      automergePersistenceLayer.pipe(Layer.provide(docs)),
      docs,
    );
  }
  ...
  static fromSaved(
    state: SavedState,
  ): Layer.Layer<Persistence | AutomergeDocs, PersistenceError> {
